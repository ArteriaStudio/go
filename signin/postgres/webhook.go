...
func LookupSubscribe(pDatabase *sql.DB, eAuthority int, pSubscribeId string, pClientState string, pUniqueId *string, pAccountId *string) (int, error) {
	if pDatabase == nil {
	...
	for pRows.Next() {
		...
		var ExpireStamp string
		if pError = pRows.Scan(pUniqueId, pAccountId, &pResourceId, &ExpireStamp); pError != nil {
			if pPQError, ok := pError.(*pq.Error); ok {
				pSQLError := pPQError.Code.Name()
				...
				return 0, fmt.Errorf("LookupSubscribe(): サブスクリプションの入力に失敗しました。pSubscribeId=[%s],ExpireStamp=[%s],SQLError=[%s]", pSubscribeId, ExpireStamp, pSQLError)
			}
			return 0, fmt.Errorf("LookupSubscribe(): サブスクリプションの入力に失敗しました。pSubscribeId=[%s],ExpireStamp=[%s]", pSubscribeId, ExpireStamp)
		} else {
			nRows++
			break
		}
...
// UpdateSubscribe 変更通知情報を更新
func UpdateSyncTimestamp(pDatabase *sql.DB, eAuthority int, pAccountId string, pResourceId string, pSubscribeId string, pSyncTimestamp string) (int64, error) {
	// トランザクションを開始
	pTx, pError := pDatabase.Begin()
	if pError != nil {
		// トランザクションの開始に失敗
	...
	// サブスクリプションの記録
	const pSQL = "UPDATE TWebHooks SET pSyncTimestamp = $1 WHERE AuthorityId = $2 AND AccountId = $3 AND ResourceId = $4;"
	pResult, pError := pTx.Exec(pSQL, pSyncTimestamp, Authoritys[eAuthority], pAccountId, pResourceId)
	if pError != nil {
		if pPQError, ok := pError.(*pq.Error); ok {
			pSQLError := pPQError.Code.Name()
			if strings.Compare(pSQLError, "unique_violation") == 0 {
