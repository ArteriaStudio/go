...
func HandlerHome(w http.ResponseWriter, r *http.Request, c *FunctionContext) {
	...
	if pError != nil {
		...
		if c.pG.pAToken.AccessToken != "" {
			...
			fmt.Fprintln(w, `<a href="/subscribe/google">サブスクリプションを登録</a>`)
			fmt.Fprintln(w, `<a href="/perm/google/revoke">アクセス許可を破棄</a>`)
		} else {
			fmt.Fprintln(w, "<a href='/perm/google/grant'>アクセス許可を申請</a>")
			fmt.Fprintln(w, `<a href="/perm/google/revoke">アクセス許可を破棄</a>`)
		}
		fmt.Fprintf(w, `</div>`)
		fmt.Fprintln(w, "</p>")
		...
		if c.pE.pAToken.AccessToken != "" {
			...
			fmt.Fprintln(w, `<a href="/subscribe/entra">サブスクリプションを登録</a>`)
			fmt.Fprintln(w, `<a href="/perm/entra/revoke">アクセス許可を破棄</a>`)
		} else {
...
func WriteBodyEntraCalendar(w http.ResponseWriter, r *http.Request, c *FunctionContext) {
	...
	entraapi.GetCalendars(r, &c.pE.pAToken, &pCalendar)
...
func HandlerGoogleSubscribe(w http.ResponseWriter, r *http.Request, c *FunctionContext) {
	...
	log.Println("pExpireTimeStamp = " + pExpireTimeStamp.String())
	pSubscriptionId := ""
...
func GetIconStream(ctx context.Context, pURL string, pSToken *oauth2.Token) ([]byte, string, error) {
	// HTTPリクエスト
	req, err := http.NewRequestWithContext(ctx, "GET", pURL, nil)
	if err != nil {
		return nil, "", fmt.Errorf("リクエストの作成に失敗: %w", err)
	}
	req.Header.Add("Authorization", "Bearer "+pSToken.AccessToken)
