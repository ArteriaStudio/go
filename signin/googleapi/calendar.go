...
func GetCalendars(r *http.Request, pAccessToken *oauth2.Token, pCalenderInfo *CalendarInfo) error {
	// Access Tokenを使用してユーザー情報を取得
	pEndPoint := "https://www.googleapis.com/calendar/v3/users/me/calendarList/Primary"
	pRequest, pError := http.NewRequest("GET", pEndPoint, nil)
	if pError != nil {
...
func GetCalendarEvents(r *http.Request, pAccessToken *oauth2.Token, pCalendarId string, pCalendarEventInfo *EventsListResponse) error {
	...
	pEndPoint := u.String() // 正しくエンコードされたURL
